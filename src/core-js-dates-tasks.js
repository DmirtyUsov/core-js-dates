...
function getNextFriday(date) {
  const friday = 5;
  const shift = ((friday + 6 - date.getDay()) % 7) + 1;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const dateLast = new Date(dateEnd);
  const diffSeconds = (dateLast.getTime() - dateFirst.getTime()) / 1000;
  return Math.round(diffSeconds / 60 / 60 / 24) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const between = new Date(date).getTime();
...
function getCountWeekendsInMonth(month, year) {
  ...
  const dateOfMonth = new Date(year, monthNum, 1);
  let countOfWeekendDay = 0;
  while (dateOfMonth.getMonth() === monthNum) {
    const weekend = [0, 6].includes(dateOfMonth.getDay()) ? 1 : 0;
    countOfWeekendDay += weekend;
    dateOfMonth.setDate(dateOfMonth.getDate() + 1);
  }
  return countOfWeekendDay;
}
...
function getWeekNumberByDate(date) {
  ...
  const january1Day = january1.getDay();
  const daysToNextMonday = january1Day === 0 ? 1 : 1 + ((7 - january1Day) % 7);
  const week2Start = new Date(date.getFullYear(), 0, 1 + daysToNextMonday);
  const msecToDay = 1000 * 60 * 60 * 24;
  const diffDays = (date - week2Start) / msecToDay + 1;
  const weekNumber = diffDays < 1 ? 1 : 1 + Math.ceil(diffDays / 7);
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  const daysToNextFriday = 5 - day;
  nextFriday.setDate(nextFriday.getDate() + daysToNextFriday);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const checkEnough = () => currentDate.valueOf() > end.valueOf();
  const setNextDay = (daysToAdd = 1) =>
    currentDate.setDate(currentDate.getDate() + daysToAdd);

  while (!checkEnough()) {
    for (let work = 0; work < countWorkDays; work += 1) {
      if (checkEnough()) {
        break;
      }
